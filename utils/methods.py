...
def test_prompt_reconstruction(model, tokenizer, sentences, seed=8, **kwargs):
    ...
    for layer_idx in range(model.config.num_hidden_layers):
        ...
        for prompt in tqdm(sentences, desc="Processing prompts"):
            set_seed(seed)
            h_target = extract_hidden_states_prompt(
                prompt, model, tokenizer, layer_idx=layer_idx
            )
        ...
        metrics.append(
            compute_metrics(
                sentences, sentense_embeddings, output_sentences, output_embeddings
            )
        )
...
def gd_all_tokens(
    ...
    **kwargs,
    ...
    optimized_embeddings = (
        embedding_matrix[random_ids].clone().detach().requires_grad_(True)
    )
    ...
    with tqdm(total=n_iterations, desc="Inverting prompt") as pbar:
        for iteration in range(n_iterations):
            current_outputs = model(
                inputs_embeds=optimized_embeddings, output_hidden_states=True
            )
            ...
            if iteration % log_freq == 0 or iteration == n_iterations - 1:
                with torch.no_grad():
                    dist = torch.cdist(
                        optimized_embeddings.squeeze(0), embedding_matrix
                    )
                    projected_ids = torch.argmin(dist, dim=1)
                    output_sentence = tokenizer.decode(
                        projected_ids.tolist(), skip_special_tokens=True
                    )
                    m = compute_metrics(
                        [input_sentence],
                        [input_embeddings],
            ...
            pbar.set_postfix(
                {
                    "Loss": loss.item(),
                    "L2": logs[-1]["l2_distance"].item() if logs else None,
                }
            )
            pbar.update(1)
    ...
    reconstructed_prompt = tokenizer.decode(
        projected_ids.tolist(), skip_special_tokens=True
    )
...
def exhaustive_search(
    model, tokenizer, prompt, layer_idx=0, seed=42, eps=1e-3, **kwargs
):
    """
    ...
    for i in range(input_ids.shape[1]):
        ...
        best_token = None
        bar = tqdm(
            range(embedding_matrix.shape[0]),
            desc=f"Finding token {i+1}/{input_ids.shape[1]}",
        )
        ...
        for j in bar:
            ...
            if loss < min_loss:
                ...
                best_token = j
            bar.set_postfix(
                {"loss": min_loss.item(), "token": tokenizer.decode(best_token)}
            )
            if loss < eps:
                break
...
def find_token(
    ...
    for _ in bar:
        ...
        string_so_far = tokenizer.decode(
            discovered_ids + [token_id], skip_special_tokens=True
        )
        bar.set_postfix_str(
            f"Loss: {loss:.2e} - Gradient norm: {grad_norm:.2e} - String: {string_so_far}"
        )
...
def gd_sequential(
    model, tokenizer, prompt, layer_idx, optimizer_cls, lr, seed=8, **kwargs
):
    """
