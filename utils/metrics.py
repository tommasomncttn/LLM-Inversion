...
def _agg_metric_over_time(metric_dict, metric_name, window_size=1):
    ...
    assert (
        time_name in metric_dict
    ), f"Metrics must contain '{time_name}' key for aggregation of '{metric_name}'."
    assert (
        metric_name in metric_dict
    ), f"Metric '{metric_name}' not found in metric_dict."

    times_metric = np.array(
        list(zip(metric_dict[time_name].flatten(), metric_dict[metric_name].flatten()))
    )
...
def compute_metrics(
    ...
    for i in range(len(target_sentences)):
        if (
            len(output_sentences[i]) < len_treshold
            or len(target_sentences[i]) < len_treshold
        ):
            continue
        _add_metric_result(
            metrics,
            "bleu",
            sentence_bleu([output_sentences[i].split()], target_sentences[i].split()),
        )
        P, R, F1 = bert_scorer.score([output_sentences[i]], [target_sentences[i]])
        _add_metric_result(metrics, "bertscore_f1", F1.item())
        try:
            rouge_s = rouge.get_scores(output_sentences[i], target_sentences[i])[0]
        ...
        _add_metric_result(
            ...
            "l2_distance",
            np.linalg.norm(
                sentense_embeddings[i] - output_embeddings[i], axis=1
            ).mean(),
        )
